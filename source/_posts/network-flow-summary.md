...
### [LuoguP4174 - 最大获利](https://www.luogu.com.cn/problem/P4174)
> 有 $n$ 个中转站，建立第 $i$ 个需要成本为 $p_i$。有 $m$ 个用户，第 $i$ 个需要同时使用中转站 $a_i$ 和 $b_i$，满足要求可获得 $c_i$ 的贡献。
> 求最大收益。

本题符合**最大权闭合子图**的性质，闭合子图满足没有指向图以外的边，即选择一个点以后它的出边指向的点也必须选择。图的点权有正有负，求点权和最大的闭合子图。具体到这一道题，每选择一个用户我们就需要建立两个中转站，花费 $p$ 的代价的同时获得 $c$ 的收益。中转站的点权为负，用户的点权为正。

下面来看求法。建立源点 $s$ 与汇点 $t$，我们要将原图转换为一个网络。首先原图中的边必须保留，因此容量为 $\infty$。从 $s$ 向每个正权点连一条容量为点权的边，从每个负权点向 $t$ 连一条容量为点权绝对值的边，用正权点权值和减去最小割即为答案。

为什么这样是对的？我们考虑最小割的性质。割掉最小割中的边后 $s$ 与 $t$ 不联通，因为原图中的边权全部为 $\infty$，所以选择了一个点以后，出边的点也必须选择，不可能被割掉。因此我们满足了闭合子图的限制。假设先满足所有用户的要求，之后对每个用户我们只有两种选择，要么放弃用户的收益，要么花钱建立中转站，这也就对应了图中连的两种边。只有割掉其一后才能保证 $s$ 与 $t$ 不连通。因此答案就是正权点权值和减去最小割。  

**[标程](https://yumomeow.github.io/2025/02/24/std/#LuoguP4174)**

### [LuoguP5996 - Muzeum](https://www.luogu.com.cn/problem/P5996)
> 二维平面上有 $n$ 件手办价值为 $v_i$ 和 $m$ 个警卫，警卫的视角是朝着 $y$ 轴负方向的扇形，扇形的一半的正切值为 $\frac{w}{h}$。警卫视角内的手办不能拿走。可以花 $v_j$ 的钱贿赂警卫使他闭上眼睛，这样就可以拿手办了。求最大的收益。  
>$n,m\le 2\times 10^5$

用这道题讲一下**模拟最大流**。建图方式很显然，警卫向能看到的手办连一条无穷边，源点向警卫、汇点向手办分别连容量为贿赂和价值的边，求最小割即可。

有两个问题：
- 警卫的视角是扇形，不方便计算看到的手办
- 数据范围太大，没法直接求最小割

对于第一个问题，我们考虑转换坐标系。对于警卫 $i$ 和手办 $j$，仅当
$$
...
这样我们就可以用扫描线求解，用 set 维护手办的坐标和剩余流量，从左往右扫描求最大流。对每个警卫**贪心地取 $y$ 最小的手办的流量**直到警卫和 $t$ 之间的边流满或找不到满足的手办，因为更大的 $y$ 可以被更多的警卫取到。这里我们直接二分查找满足条件的手办。通过**模拟最大流**成功解决了数据范围的问题。

**[标程](https://yumomeow.github.io/2025/02/24/std/#LuoguP5996)**
...
### [LuoguP4043 - 支线剧情](https://www.luogu.com.cn/problem/P4043)
> 给你一个 DAG，每条边有一个边权，每次必须从 $1$ 号点出发花费路径上边权之和的费用走一条路径，求每条边至少覆盖一次的最少费用。

**有源汇最小费用可行流**问题。每条边流量下界为 $1$，上界为 $\infty$，求最小费用。设原图中源点与汇点分别为 $s,t$，按如下方式建边：

- 建立附加源点 $S$，和附加汇点 $T$。
- 对于原图中每一个点（包括源汇）$u$,令 $d_u$ 代表 $u$ 点的所有入边的流量下界减去出边的流量下界。如果 $d_u$ 是负数，连边 $(u,T,-d_u,0)$。如果 $d_u$ 是正数，连边 $(S,u,d_u,0)$。
- 对于原图中每一条边，连边 $(u,v,r-l,w)$。
- 连边 $(t,s,\infty,0)$。

求出最小费用最大流后，加上每条边下界 $\times$ 费用即为答案。

**[标程](https://yumomeow.github.io/2025/02/24/std/#LuoguP4043)**
